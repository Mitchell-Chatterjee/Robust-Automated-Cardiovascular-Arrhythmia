...
'''
Except for the sampled noise transformations. These transformations have been taken from: 

Mehari, Temesgen, and Nils Strodthoff. "Self-supervised representation learning from 12-lead ECG data." 
Computers in biology and medicine 141 (2022): 105114.
'''
...
def Tinterpolate(data, marker):
    ...
    data = data.flatten()
    interpolation = np.interp(np.where(data == marker)[0], np.where(data != marker)[0], data[data != marker])
    data[data == marker] = interpolation.astype(data.dtype)
...
def refill(arr, center, radius, step):
    ...
    arr[center - radius * step - radius:center - radius * step, :] = arr[center +
                                                                         radius * step + 1:center + radius * step + radius + 1,
                                                                     :] = np.inf
...
class TNoTransform(Transformation):
    '''Empty Transformation to be applied
    '''

    def __init__(self):
        super(TNoTransform).__init__()
...
class TSampledNoise(Transformation):
    ''' Add sampled noise (These transformations are unique to our work)
    '''

    def __init__(self, root_path_noise, leads):
        super(TSampledNoise, self).__init__()
        self.noise_dataset = EcgNoiseDataset(root_path=root_path_noise, number_of_leads=leads)

    def __call__(self, sample):
        data, label = sample
        leads, seq_len = data.shape
        # Select random number of leads to augment with noise
        lead_indices = np.random.choice(leads, np.random.choice(leads), replace=False)

        # Get noise batch and multiply by random scale
        noise_batch = np.random.uniform(0, 1) * self.noise_dataset.get_noise_batch(batch_size=1, seq_len=seq_len)

        # Add noise to data
        data[lead_indices, :] += noise_batch[lead_indices, :]
...
class TGaussianNoise(Transformation):
    ...
    def __call__(self, sample):
        if self.scale == 0:
            ...
            data, label = sample
            data = data + self.scale * np.random.randn(data.shape[0], data.shape[1])
            return data, label
...
class TRandomResizedCrop(Transformation):
    ...
    def __init__(self, crop_ratio_range=[0.5, 1.0], output_size=1000):
        super(TRandomResizedCrop, self).__init__(
            crop_ratio_range=crop_ratio_range, output_size=output_size)
        self.crop_ratio_range = crop_ratio_range

    def __call__(self, sample):
        output = torch.full(sample[0].shape, torch.inf)
        channels, timesteps = output.shape
        crop_ratio = random.uniform(*self.crop_ratio_range)
...
class TChannelResize(Transformation):
    ...
    def __call__(self, sample):
        data, label = sample
        channels, timesteps = data.shape
        resize_factors = torch.exp(torch.empty(channels).uniform_(*self.log_magnitude_range)).reshape(channels, 1)
        resize_factors_same_shape = np.repeat(resize_factors, timesteps, axis=1)
...
class ToTensor(Transformation):
    ...
    def __init__(self, transpose_data=False, transpose_label=False):
