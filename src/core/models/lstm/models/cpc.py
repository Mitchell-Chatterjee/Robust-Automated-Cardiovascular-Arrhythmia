...
from src.core.models.lstm.clinical_ts.basic_conv1d import _conv1d
import numpy as np

from src.core.models.lstm.clinical_ts.basic_conv1d import listify, bn_drop_lin
...
class CPCModel(nn.Module):
    "CPC model"
    def __init__(self, input_channels, mode: Mode, strides=[5,4,2,2,2], kss=[10,8,4,4,4], features=[512,512,512,512],
                 bn_encoder=False, n_hidden=512,n_layers=2,mlp=False,lstm=True,bias_proj=False,
                 num_classes=None, concat_pooling=True, ps_head=0.5,lin_ftrs_head=[512],bn_head=True,
                 skip_encoder=False, linear_only=False):
        super().__init__()
        assert(skip_encoder is False or mode is not Mode.PRETRAIN)#pretraining only with encoder
        ...
        if mode == Mode.PRETRAIN: #pretraining
            if(mlp):# additional hidden layer as in simclr
                self.proj_head = nn.Sequential(nn.Linear(n_hidden, n_hidden),nn.ReLU(inplace=True),nn.Linear(n_hidden, self.encoder_output_dim,bias=bias_proj))
            else:
                self.proj_head = nn.Linear(n_hidden, self.encoder_output_dim,bias=bias_proj)
        elif self.linear_only:
            self.head = nn.Linear(n_hidden, num_classes)
        else:
            #classifier
    ...
    def forward(self, input):
        ...
        if self.mode == Mode.PRETRAIN:#pretraining
            return input_encoded, self.proj_head(output_rnn)
        else:#classifier
    ...
    def cpc_loss(self, input_encoded, output, target=None, steps_predicted=12, n_false_negatives=128, negatives_from_same_seq_only=True, eval_acc=False):
        #input_encoded: bs, seq, features; output: bs,seq,features
        ...
        loss = torch.tensor(0,dtype=torch.float32).to(input_encoded.device)
        tp_cnt = torch.tensor(0,dtype=torch.int64).to(input_encoded.device)
        ...
        for i in range(input_encoded.size()[1]-steps_predicted):
            ...
            if(negatives_from_same_seq_only):
                idxs = torch.randint(0,(seq-1),(bs*n_false_negatives,)).to(input_encoded.device)
            else:#negative from everywhere
                idxs = torch.randint(0,bs*(seq-1),(bs*n_false_negatives,)).to(input_encoded.device)
            ...
            if(negatives_from_same_seq_only):
                idxs_batch = torch.arange(0,bs).repeat_interleave(n_false_negatives).to(input_encoded.device)
            else:
            ...
            targs = torch.zeros(bs, dtype=torch.int64).to(input_encoded.device)
