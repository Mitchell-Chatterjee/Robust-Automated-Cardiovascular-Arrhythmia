...
#SBATCH --array=1-3%1       # 3 is the number of jobs in the chain. Gives approximately 12 epochs.
#SBATCH --nodes=1
...
#SBATCH --output=cpc_model_code_dataset-%N.out
...
srun python -m src.patchtst_pretrain \
          ...
          --head_dropout=0. \
          --weight_decay=0.001 \
          --data_augmentation=pretrain_cpc \
          --no-scheduler \
          --custom_lead_selection=all_leads \
          --trafos=None \
          --model=cpc \
          --model_name=cpc_model_code_dataset
